...
public class WebSocketHuobi extends WebSocketClient {
    ...
    public WebSocketHuobi(URI uri, KlineRobotMarketService service) {
        ...
        Proxy proxy = new Proxy(Proxy.Type.HTTP, new InetSocketAddress("172.31.170.2",13080));
        logger.info(String.format("设置代理数据为: %s :%d", "172.31.170.2",13080));
        this.setProxy(proxy);
    }
        ...
        try {
            ...
            if (!"".equals(message)) {
                if (message.indexOf("ping") > 0) {
                    ...
                    if(jsonObject.containsKey("err-msg")){
                        logger.error(String.format("火币返回异常{%s}", jsonObject.toJSONString()));
                    }
                    String id = "";
